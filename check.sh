...
# Check if the original list of sha512sums of the /boot and /efi partitions is present otherwise it will create one
 ...
 then
    ...
    sudo find /boot -type f -exec sha512sum "{}" + | sudo tee ./sha512sum_list_boot_orig.txt >/dev/null
fi
 ...
 then
   ...
   sudo tail -n +2 '/sec/bios_info_orig.txt' | sudo tee temp.tmp >/dev/null && sudo mv temp.tmp '/sec/bios_info_orig.txt' >/dev/null
 fi
 ...
 then
   ...
   sudo lshw -class disk | grep $ssd -A 5 -B 5 | sudo tee ./ssd_orig.txt >/dev/null
 fi
 ...
 then
   ...
   sudo tpmtool eventlog dump | grep -A 3 'PCR: 0' | sudo tee ./tpm.bootguard_orig.txt >/dev/null
 fi
 ...
 then
   ...
   echo "Extract tpm infos"
   sudo tpmtool eventlog dump | grep -E -A 1 -B 2 "initrd /" | sudo tee ./tpm.kernel_orig.txt >/dev/null
 fi
#------------------------------------------Compare old to new---------------------------------------------------------------#
echo "////////////////////////////////////////////////////////////////////////////"
...
sudo find /boot -type f -exec sha512sum "{}" + | sudo tee ./sha512sum_list_boot_new.txt >/dev/null
...
echo "Extract ssd infos"
sudo lshw -class disk | grep $ssd -A 5 -B 5 | sudo tee ./ssd_new.txt >/dev/null
...
echo "Extract tpm infos"
sudo tpmtool eventlog dump | grep -A 3 'PCR: 0' | sudo tee ./tpm.bootguard_new.txt  >/dev/null
...
echo "Extract tpm infos"
sudo tpmtool eventlog dump | grep -E -A 1 -B 2 "initrd /" | sudo tee ./tpm.kernel_new.txt >/dev/null
